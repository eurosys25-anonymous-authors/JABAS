  ...
  py::class_<Module, Object>(m, "ScriptModule")
      ...
      .def(
          ...
          [](Module& self,
            ...
            // std::cout << "[torch/csrc/jit/python/script_init.cpp] _create_method_from_trace()" << std::endl;
            // std::cout << "method name: " << name << std::endl; // forward
            ...
            int new_id = -1;
            for (const auto& opnode : nodes) {
              std::cout << "[DEBUG][torch/csrc/jit/python/script_init.cpp] node id: "
                        << ++new_id
                        << ": opnode name: "
                        << opnode->kind().toQualString() 
                        << std::endl;
            }
            ...
            std::cout << "[DEBUG][torch/csrc/jit/python/script_init.cpp] input_num: "
                      << input_num << std::endl;
            ...
            int new_id2 = -1;
            for (const auto& opnode : root_nodes) {
              std::cout << "[DEBUG][torch/csrc/jit/python/script_init.cpp] node id: "
              << ++new_id2
              << ": opnode name: "
              << opnode->kind().toQualString() 
